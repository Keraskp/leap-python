...
class LeapMotionListen(Leap.Listener):
    def on_init(self, controller):
        ...
        self.data = []
        self.scaler = pickle.load(open('sklearn_scaler/scaler.pkl', 'rb'))
        self.model = pickle.load(open("sklearn_models/svm_rbf.pkl", "rb"))
    ...
    def on_frame(self, controller):
        ...
        # scal = self.scaler.fit(frame_data)
        frame_data = self.scaler.transform(frame_data)
        # print(frame_data)
        result = self.model.predict(frame_data)
        print(self.gestures[int(result[0])])
